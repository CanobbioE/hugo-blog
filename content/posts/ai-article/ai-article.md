...
showDate: false
draft: false
tags: ["blog","story", "LLM", "AI", "machine learning"]
---

We all heard all about LLMs. So I thought to myself: why not make one of these large language models write a blog post for me?

It all started with a simple prompt:

```
Hey LLM, write me an article on how AI can be used to write articles.
```
...
I quickly scan through it, and the first thing that annoyed me was the redundancy and the endless repetition of the same concept in different formats.

Obviously it's my fault for not giving enough information in the initial prompt for the glorified autocomplete to write about it.
The article does make some decent points and most importantly is grammatically correct. Speaking of which:
...
If that's still too much text, do not worry because:
...
In conclusion, large language models are fun to toy with, but I still have to type all my posts (and my code) myself.
In all seriousness, LLMs are powerful tools that really help with simple but time consuming tasks.
However, I feel that relying too much on LLMs is quite dangerous, especially when used in fields we do not understand: always double-check an LLM output.